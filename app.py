...
# Attempt to find the same block in original tex
old_env_match = re.findall(r"(\begin{.*?}.*?\end{.*?})", tex, flags=re.S)
if old_env_match:
tex = replace_environment_block(tex, block.split('{')[1].split('}')[0], old_env_match[0], block)
return tex
